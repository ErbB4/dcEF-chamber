...
data = pd.read_csv("calcium_data_with-metal-disc.csv")
data =data.dropna()
...
# plot boxplot
sns.barplot(data=data_stim,x="timing",y="normed_firing_rate",order=["1st","2nd"],palette=["#ffbb96ff","#3d5a80ff"],capsize=0.1,linewidth=0.5,ax=ax1)
sns.barplot(data=data_sham,x="timing",y="normed_firing_rate",order=["1st","2nd"],palette=["#ffbb96ff","#98c1d9ff"],capsize=0.1,linewidth=0.5,ax=ax2)


### add stats ###
# stats were performed and uniformally documented in the lab using GraphPad 
ax1.text(1,1.6,"***",fontsize=12,ha='center',va='bottom')
...
ax1.set_xticklabels(["1st","dcEF"])
ax2.set_xticklabels(["1st","sham"])
ax1.set_title("dcEF")
ax2.set_title("sham")
...
plt.savefig("calcium-with-metal-disc.svg")
